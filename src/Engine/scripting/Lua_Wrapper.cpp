...
void Lua_Wrapper::RegisterCoreFunctions(lua::State* state)
{
    ...
    state->set("isCollidingRR",&sysPhysics_Wrapper::isCollidingRR);
    state->set("isCollidingRC",&sysPhysics_Wrapper::isCollidingRC);
    state->set("isCollidingCP",&sysPhysics_Wrapper::isCollidingCP);
